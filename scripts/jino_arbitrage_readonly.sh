...
CONFIG_NAME="conf_jino_readonly.yml"
AMOUNT="${JINO_READONLY_QUOTE_AMOUNT:-25}"
...
hbot create jino_cross_exchange_arbitrage --controller \
  ...
  --set safety_mode=readonly \
  --set total_amount_quote="$AMOUNT" \
  --set max_trade_amount_quote="$AMOUNT"

echo "Starting Jino AUTHENTICATED READ-ONLY mode."
echo "This mode can NEVER create an executor/order."
echo "Configure Binance/KuCoin API keys with READ permission only."
echo "Spot trading OFF. Withdrawals OFF."
